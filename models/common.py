...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        print(F'conf- c1={c1}, c2={c2}, k={k}, s={s}, g={g}, p={p}')
...
# def epnet_block(inputs, num_filters, compression_factor=4, maxpool=3, stride=1, attention=True, attention_k=3, batchnorm=False, dropout_rate=0.25, skip_tensor_in=None):
#     x=inputs
#     if compression_factor>1:
#       x=tf.keras.layers.Conv2D(num_filters//compression_factor, 1, padding="same", activation=None, use_bias=False)(x)
#     if skip_tensor_in is not None:
#       _, skip_rows, skip_cols,_ = skip_tensor_in.get_shape().as_list()
#       _, in_rows, in_cols, _ = x.get_shape().as_list()
#       avg_pool_stride=(skip_rows//in_rows, skip_cols//in_cols)
#       s = tf.keras.layers.AveragePooling2D(pool_size=avg_pool_stride)(skip_tensor_in)
#       s = tf.keras.layers.Conv2D(num_filters//compression_factor, 3, padding="same", activation=None, use_bias=False)(s)
#       x = x+s

#     x=tf.keras.layers.Conv2D(num_filters, attention_k, strides=(stride, stride), padding="same", activation='relu', use_bias=False)(x)
#     if attention:
#         y=tf.keras.layers.Conv2D(num_filters, 3, padding="same", activation='sigmoid', use_bias=False)(x)
#         x=x*y
#     if maxpool>1:
#         x=tf.keras.layers.MaxPool2D((maxpool, maxpool))(x)
#     if dropout_rate>0:
